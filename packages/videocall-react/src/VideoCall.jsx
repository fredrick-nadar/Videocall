...
/**
 * VideoCall Component - Full-featured video calling with WebRTC
 * 
 * @param {string} serverUrl - Backend server URL (required)
 * @param {string} username - Pre-filled username (optional)
 * @param {function} onCallEnd - Callback when call ends (optional)
 */
export default function VideoCall({ serverUrl, username: initialUsername = "", onCallEnd }) {
    
    if (!serverUrl) {
        throw new Error("VideoCall component requires 'serverUrl' prop");
    }
    ...
    let connectToSocketServer = () => {
        ...
        socketRef.current.on('connect', () => {
            const storedUsername = localStorage.getItem("username") || username || "Anonymous";
                            ...
                            <CardContent sx={{ padding: '40px' }}>
                                ...
                                <Box sx={{ mb: 3 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mb: 2 }}>
                                        Meeting Code: <strong>{window.location.pathname.substring(1) || 'default-room'}</strong>
                                    </Typography>
                                </Box>
                        ...
                        <Box className={styles.lobbyContent}>
                            <Card className={styles.lobbyCard} elevation={8}>
                                <CardContent sx={{ padding: '40px', textAlign: 'center' }}>
                                    ...
                                    <Button 
                                        variant="contained" 
                                        onClick={() => onCallEnd ? onCallEnd() : (window.location.href = "/")}
                                        sx={{ 
                                            mt: 2,
                                            bgcolor: '#000',
                                            '&:hover': { bgcolor: '#333' }
